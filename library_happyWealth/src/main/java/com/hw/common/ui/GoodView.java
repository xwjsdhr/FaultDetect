...
public class GoodView extends PopupWindow{
    ...
    private String mText = ""; // 默认文本;

    private int mTextColor = Color.BLACK;   // 默认文本字体颜色;

    private int mTextSize = 16; // 默认文本字体大小;

    private int mFromY = 0; // Y轴移动起始偏移量;

    private int mToY = DISTANCE; // Y轴移动最终偏移量;

    private float mFromAlpha =  1.0f; // 起始时透明度;

    private float mToAlpha = 0.0f;  // 结束时透明度;

    private int mDuration = DURATION;

    private int mDistance = DISTANCE; // 动画时长;
